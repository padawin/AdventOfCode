...
def get_weights(nodes):
    ...
    def get_weight_node(nodes, node):
        weight_children = [
            get_weight_node(nodes, n)
            if n not in weights else weights[n]
            for n in nodes[node][1]
        ]
        total_weight = sum(weight_children)
        distinct_weights = set(weight_children)
        if len(distinct_weights) > 1:
            print(node, weight_children, nodes[node][1])
            print(distinct_weights.pop() - distinct_weights.pop())

        return nodes[node][0] + total_weight

    for node in nodes:
        weights[node] = get_weight_node(nodes, node)


print("test data:")
get_weights(test)
print("actual data:")
get_weights(input_vals)
