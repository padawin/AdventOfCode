...
reg = '^([a-z]+) (inc|dec) (-?\d+) if ([a-z]+) (==|<|<=|!=|>=|>) (-?\d+)$'


class Analyser(object):
    def comp_register(self, register, comp, value):
        curr = 0 if register not in self.registers else self.registers[register]
        if comp == '==':
            return curr == value
        elif comp == '<':
            return curr < value
    ...
    def analyse_instruction(self, instruction):
        m = re.search(reg, instruction)
        register = m.group(1)
        operation = 1 if m.group(2) == 'inc' else -1
        value = int(m.group(3))
        condition_register = m.group(4)
        comparison = m.group(5)
        threshold = int(m.group(6))

        if register not in self.registers:
            self.registers[register] = 0

        do_action = self.comp_register(
            condition_register,
            comparison,
            threshold
        )
